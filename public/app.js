/**
 * ═══════════════════════════════════════════════════════════════
 *  ARYADMONEY PWA — app.js  (version corrigée)
 *  Router SPA, views, UI components
 * ═══════════════════════════════════════════════════════════════
 */
...
/* ─── Country state (CORRECTION BUG PRINCIPAL) ───────────────── */
/* currentCountry stocke l'id du champ ET le code pays sélectionné */
const currentCountry = {
  id:   'login',   /* champ actif : login | reg | vir | rch | bene */
  code: 'MA',      /* code ISO par défaut */
};
...
Router.register('welcome', () => {
  /* Redirige automatiquement après que la vue est rendue */
  setTimeout(() => {
  ...
  }, 100); /* court délai — le splash géré dans index.html */
  return `<div style="height:100dvh;display:flex;align-items:center;justify-content:center;">
    <div style="text-align:center;">
      <div style="font-size:28px;font-weight:900;background:linear-gradient(135deg,var(--gold),var(--gold-dark));
                  -webkit-background-clip:text;-webkit-text-fill-color:transparent;">AryadMoney</div>
    ...
    <div class="auth-logo">
      <div class="auth-logo-mark">
        <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
          <path d="M18 6C11.4 6 6 11.4 6 18S11.4 30 18 30 30 24.6 30 18 24.6 6 18 6Z"
                fill="white" fill-opacity="0.2"/>
          <path d="M12 18H24M18 12V24" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
        </svg>
      </div>
      <div class="auth-logo-text">AryadMoney</div>
    </div>
    ...
    <div class="auth-form stagger">
      ...
      <div class="input-group">
        ...
        <div class="input-with-icon">
          ...
          <button class="input-action" onclick="togglePassword('login-pass', this)">
      ...
      <button class="btn btn-ghost" style="font-size:13px;justify-content:flex-end;padding:0;"
        onclick="showForgotPassword()">Mot de passe oublié ?</button>
...
Router.register('register', () => `
  <div class="auth-page">
    ...
    <h1 class="auth-title" style="margin-top:8px;">Créer un compte</h1>
    ...
    <div class="auth-form stagger">
      ...
      <div class="input-group">
        ...
        <div class="input-with-icon">
          ...
          <input class="input" id="reg-pass" type="password" placeholder="Minimum 6 caractères" />
...
Router.register('confirmation', (params) => {
  ...
  <div class="auth-page centered">
    ...
    <div style="width:72px;height:72px;background:var(--gold-faint);border:2px solid rgba(218,174,84,0.3);
    ...
    <p class="auth-subtitle" style="max-width:280px;">
      Un code a été envoyé à<br>
      <strong style="color:var(--text-primary);">${email}</strong>
    </p>
    <div class="otp-group" id="otp-group" style="margin:28px 0 8px;">
    ...
    <button class="btn btn-primary" id="otp-btn"
      onclick="doVerifyOtp('${phone}','${pays}')" style="max-width:320px;margin-top:8px;">
      <span id="otp-btn-text">Valider</span>
    </button>
    <button class="btn btn-ghost" id="resend-btn"
      onclick="doResendOtp('${phone}','${pays}')" style="font-size:13px;max-width:320px;margin-top:8px;">
      Renvoyer le code
    </button>
  </div>`;
});
  ...
  <div style="min-height:100%;">
    ...
    <div class="home-content stagger">
      ...
      <div>
        ...
        <div id="home-ops">${renderOperationsSkeleton()}</div>
      </div>
    </div>
  </div>`;
});
...
Router.register('historique', () => `
  <div>
    ...
    <div id="hist-list" style="padding:0 20px 16px;">${renderOperationsSkeleton()}</div>
  </div>
`);
      ...
      <div>
        ...
        <div class="amount-display" style="background:var(--surface);border-radius:var(--radius);padding:24px 16px;text-align:center;">
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">
            Solde: ${formatMontant(AppState.user?.solde || 0, 'MAD')}
          </div>
        </div>
...
Router.register('recharge', () => `
  <div>
    ...
    <div style="padding:12px 20px 0;">
      <div class="tab-bar">
        <button class="tab-item active" data-tab="demande" onclick="switchRechargeTab(this)">💳 Demande</button>
        <button class="tab-item"        data-tab="ticket"  onclick="switchRechargeTab(this)">🎫 Ticket</button>
        <button class="tab-item"        data-tab="suivi"   onclick="switchRechargeTab(this)">📋 Suivi</button>
      </div>
    </div>
    ...
    <div id="tab-demande" style="padding:20px;display:flex;flex-direction:column;gap:20px;">
      ...
      <div id="rch-montant-section" class="hidden">
        ...
        <p style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">
          Entrez exactement le montant que vous avez envoyé — devise :
          <strong id="rch-devise-label" style="color:var(--gold);">MAD</strong>
        </p>
        <div class="input-group">
          <div style="position:relative;">
            <input class="input" id="rch-montant" type="number" placeholder="Ex: 500"
              inputmode="decimal" style="padding-right:60px;" oninput="validateMontantRecharge()" />
            <span id="rch-devise-badge" style="position:absolute;right:14px;top:50%;transform:translateY(-50%);
              font-size:13px;font-weight:700;color:var(--gold);font-family:var(--font-mono);">MAD</span>
          </div>
        </div>
      ...
      <div id="rch-ref-section" class="hidden">
        ...
        <p style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">
          Entrez la référence fournie lors du paiement
        </p>
        <div class="input-group">
          <input class="input mono" id="rch-num-transaction" type="text"
            placeholder="Ex: TXN-20240401-123456" style="letter-spacing:1px;" />
      ...
      <div id="rch-submit-section" class="hidden">
        <div style="background:var(--surface);border-radius:var(--radius);padding:14px;margin-bottom:16px;font-size:12px;color:var(--text-muted);line-height:1.6;">
          ...
          📧 Votre ticket sera envoyé par <strong style="color:var(--text-primary);">email</strong>
        </div>
    ...
    <div id="tab-ticket" style="padding:20px;display:none;flex-direction:column;gap:18px;">
      <div style="text-align:center;padding:20px 0;">
        ...
        <p style="font-size:13px;color:var(--text-muted);">Entrez le code de votre ticket reçu par email</p>
      </div>
      <div class="input-group">
        <label class="input-label">Code du ticket</label>
        <input class="input mono" id="ticket-code" type="text" placeholder="TKT-XXXXXXXXXX"
...
Router.register('retrait', () => `
  <div>
    ...
    <div style="padding:20px;display:flex;flex-direction:column;gap:20px;">
      <div style="background:var(--gold-faint);border:1px solid rgba(218,174,84,0.3);border-radius:var(--radius);
        ...
        <p style="font-size:12px;color:var(--text-secondary);line-height:1.5;">
          Vous allez générer un code de retrait. Utilisez-le auprès d'un agent agréé AryadMoney.
        </p>
      </div>
      <div>
      ...
      <div class="input-group">
        ...
        <div class="input-with-icon">
          ...
          <input class="input" id="ret-pin" type="password" maxlength="6" placeholder="••••••"
            inputmode="numeric" style="letter-spacing:6px;font-size:20px;" />
  ...
  <div>
    ...
    <div style="padding:12px 20px;">
      <div class="input-with-icon">
        ...
        <input class="input" id="bene-search" type="search"
          placeholder="Rechercher un bénéficiaire..." oninput="filterBenes()" />
      </div>
    </div>
    <div id="bene-list" style="padding:0 20px 16px;">${renderBeneListSkeleton()}</div>
  </div>
`);
...
Router.register('notification', () => `
  <div>
    <div class="page-header">
      ...
      <h1>Notifications</h1>
      <button class="header-action" onclick="markAllRead()">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="20 6 9 17 4 12"/>
        </svg>
      </button>
    </div>
    <div id="notif-list">${renderNotifSkeleton()}</div>
  </div>
`);
  ...
  <div>
    ...
    <div style="padding:0 20px 32px;">
      ...
      <div class="card" style="margin-bottom:12px;">
        ...
        <div class="settings-row" onclick="Router.go('historique')">
          <div class="settings-icon blue">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="card">
        <div class="settings-row" onclick="doLogout()">
          ...
          <div class="settings-text">
            <div class="settings-label" style="color:var(--error);">Déconnexion</div>
          </div>
        </div>
      </div>
...
function renderSoldeBanner() {
  ...
  const devise = user.devise || 'MAD';
  return `
    <div style="margin:12px 20px 0;padding:12px 16px;background:var(--surface);
...
function renderKeypad(id) {
  ...
  return `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
    ${keys.map(k => `
      <button onclick="keypadPress('${id}','${k}')"
        style="height:52px;background:var(--surface);border-radius:var(--radius);
               font-size:${k==='⌫'?'20px':'18px'};font-weight:600;color:var(--text-primary);transition:all .15s;"
        onmousedown="this.style.background='var(--surface-light)'"
        onmouseup="this.style.background='var(--surface)'"
        ontouchstart="this.style.background='var(--surface-light)'"
        ontouchend="this.style.background='var(--surface)'">${k}</button>`).join('')}
  </div>`;
}
...
function showCountryPicker(forId) {
  ...
  const html = Object.entries(COUNTRIES).map(([code, c]) => `
    <button onclick="selectCountry('${code}')"
      style="display:flex;align-items:center;gap:12px;width:100%;padding:14px 0;
             border-bottom:1px solid rgba(88,94,101,0.3);">
...
function keypadPress(id, key) {
  ...
  if (display) {
    ...
    display.classList.toggle('has-value', parseFloat(val) > 0);
  }
...
function renderHistoriqueList() {
  ...
  if (!list) return;
  if (!ops.length) {
    list.innerHTML = emptyState('receipt', 'Aucune opération', 'Aucun résultat pour ce filtre.');
    return;
  }
...
function renderOpItem(op) {
  ...
  /* Échappement simplifié pour l'attribut onclick */
  const opJson  = JSON.stringify(op).replace(/"/g, '&quot;');
  return `
    <div class="tx-item" onclick='showOpDetail(${JSON.stringify(op)})'>
...
function emptyState(icon, title, sub) {
  const icons = {
    receipt: `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                <polyline points="14 2 14 8 20 8"/></svg>`,
    notif:   `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/>
                <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>`,
    people:  `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
...
function showOpDetail(op) {
  if (typeof op === 'string') { try { op = JSON.parse(op); } catch { return; } }
...
function renderBeneList(list) {
  ...
  if (!el) return;
  if (!filtered.length) {
    el.innerHTML = emptyState('people', 'Aucun bénéficiaire', 'Ajoutez des bénéficiaires pour des virements rapides.');
    return;
  }
  el.innerHTML = filtered.map(b => {
    ...
    <div class="bene-item">
      ...
      <button onclick='startVirementToBene(${JSON.stringify(b)})'
        style="background:var(--gold-faint);color:var(--gold);border:1px solid rgba(218,174,84,0.3);
               border-radius:var(--radius-sm);padding:6px 12px;font-size:12px;font-weight:700;">Virer</button>
      <button onclick="confirmDeleteBene('${b.id}')"
        style="background:rgba(229,57,53,0.1);color:var(--error);border-radius:50%;
...
function filterBenes() { renderBeneList(AppState.beneficiaires); }
...
function showAddBene() {
  Modal.show(`
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
      <div class="input-group"><label class="input-label">Prénom</label>
        <input class="input" id="bene-prenom" placeholder="Prénom" /></div>
      <div class="input-group"><label class="input-label">Nom</label>
        <input class="input" id="bene-nom" placeholder="Nom" /></div>
      <div class="input-group">
        <label class="input-label">Téléphone</label>
        <div class="phone-row">
          <button class="country-select" id="bene-country-btn"
            onclick="showCountryPicker('bene')">🇲🇦 +212 <span style="font-size:10px;margin-left:4px;">▼</span></button>
...
function renderNotifications(list) {
  ...
  if (!el) return;
  if (!list.length) {
    el.innerHTML = `<div style="padding:20px;">${emptyState('notif','Aucune notification','Vous êtes à jour !')}</div>`;
    return;
  }
...
async function doLogin() {
  ...
  btn.disabled = true;
  text.innerHTML = '<div class="spinner"></div> Connexion...';

  /* CORRECTION : currentCountry.code est maintenant fiable */
  ...
  if (r.isSuccess) {
    ...
    Router.go('confirmation', { maskedEmail: '****@****.com', telephone: tel, codePays });
  } else {
    Toast.error(r.error);
  }
}
...
async function doRegister() {
  ...
  btn.disabled = true;
  text.innerHTML = '<div class="spinner"></div> Création...';

  const r = await AuthService.register({
    telephone: tel,
    codePays: currentCountry.code || 'MA',
    email,
    motDePasse: pass,
  });
...
async function doVerifyOtp(telephone, codePays) {
  ...
  btn.disabled = true;
  text.innerHTML = '<div class="spinner"></div> Vérification...';

  const r = await AuthService.verifyOtp({ telephone, codePays, code });

  btn.disabled = false;
  text.textContent = 'Valider';
...
function showForgotPassword() {
  Modal.show(`
    ...
    <div class="phone-row" style="margin-bottom:16px;">
      <button class="country-select" id="forgot-country-btn"
        onclick="showCountryPicker('forgot')">🇲🇦 +212 <span style="font-size:10px;margin-left:4px;">▼</span></button>
...
async function doForgotPass() {
  ...
  if (!tel) return;
  Modal.hide();
  await AuthService.forgotPassword({ telephone: tel, codePays: currentCountry.code || 'MA' });
...
async function doVirementConfirm() {
  ...
  const pays = currentCountry.code || 'MA';
  Modal.show(`
    <div class="modal-title">Confirmer le virement</div>
    <div class="confirm-amount-big">${formatMontant(montant, 'MAD')}</div>
    <div class="confirm-card">
      <div class="confirm-row">
        <span class="confirm-key">Destinataire</span>
        <span class="confirm-val mono">${COUNTRIES[pays]?.code || '+212'} ${tel}</span>
      </div>
      <div class="confirm-row">
        <span class="confirm-key">Montant</span>
        <span class="confirm-val">${formatMontant(montant,'MAD')}</span>
      </div>
      ${motif ? `<div class="confirm-row"><span class="confirm-key">Motif</span><span class="confirm-val">${motif}</span></div>` : ''}
    </div>
    <div style="display:flex;gap:12px;">
      <button class="btn btn-secondary" onclick="Modal.hide()">Annuler</button>
      <button class="btn btn-primary" id="vir-confirm-btn"
        onclick="doVirement('${tel}','${pays}',${montant},'${motif||''}')">Confirmer</button>
    </div>
  `);
}
...
async function doRetraitConfirm() {
  ...
  Modal.show(`
    ...
    <div class="confirm-amount-big">${formatMontant(montant,'MAD')}</div>
    <p style="color:var(--text-secondary);font-size:13px;text-align:center;margin-bottom:16px;">
      Un code de retrait vous sera transmis. Présentez-le à un agent agréé.
    </p>
    <div style="display:flex;gap:12px;">
      <button class="btn btn-secondary" onclick="Modal.hide()">Annuler</button>
      <button class="btn btn-primary" id="ret-confirm-btn"
        onclick="doRetrait(${montant},'${pin}')">Générer</button>
    </div>
  `);
}
  ...
  if (r.isSuccess) {
    ...
    Modal.show(`
      ...
      <div style="background:var(--bg);border-radius:var(--radius);padding:24px;text-align:center;margin-bottom:16px;">
        <div style="font-size:32px;font-weight:900;font-family:var(--font-mono);
                    letter-spacing:4px;color:var(--gold);">${code}</div>
      ...
      <button class="btn btn-ghost" onclick="Modal.hide();Router.go('home')" style="margin-top:8px;">Fermer</button>
    `);
  } else {
    Toast.error(r.error);
  }
...
async function doAddBene() {
  ...
  const r = await AccountService.addBeneficiaire({ nom, prenom, telephone: tel, pays: currentCountry.code || 'MA' });
  Modal.hide();
  if (r.isSuccess) {
...
function showEditProfile() {
  ...
  Modal.show(`
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
      <div class="input-group"><label class="input-label">Prénom</label>
        <input class="input" id="ep-prenom" value="${user.prenom||''}" /></div>
      <div class="input-group"><label class="input-label">Nom</label>
        <input class="input" id="ep-nom" value="${user.nom||''}" /></div>
      <div class="input-group"><label class="input-label">Email</label>
        <input class="input" id="ep-email" type="email" value="${user.email||''}" /></div>
...
function showChangePinModal() {
  Modal.show(`
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
      <div class="input-group"><label class="input-label">PIN actuel</label>
        <input class="input mono" id="cp-old" type="password" maxlength="6" inputmode="numeric" placeholder="••••••" /></div>
      <div class="input-group"><label class="input-label">Nouveau PIN</label>
        <input class="input mono" id="cp-new" type="password" maxlength="6" inputmode="numeric" placeholder="••••••" /></div>
      <div class="input-group"><label class="input-label">Confirmer PIN</label>
        <input class="input mono" id="cp-conf" type="password" maxlength="6" inputmode="numeric" placeholder="••••••" /></div>
...
/* ══════════════════════════════════════════════════════════════
   POST-RENDER HOOKS
══════════════════════════════════════════════════════════════ */
document.addEventListener('view:rendered', async ({ detail: { route } }) => {
  ...
  if (route === 'home') {
    ...
    if (homeOps) {
      homeOps.innerHTML = ops.length
        ? ops.map(renderOpItem).join('')
        : emptyState('receipt', 'Aucune opération', 'Vos transactions apparaîtront ici.');
    }
  }
  ...
  if (route === 'virement') {
    ...
    if (r.isSuccess) {
      ...
      if (benes) {
        benes.innerHTML = AppState.beneficiaires.length === 0
          ? '<span style="font-size:12px;color:var(--text-muted);">Aucun bénéficiaire enregistré.</span>'
          : AppState.beneficiaires.slice(0,6).map(b => `
              <button onclick='quickSelectBene("${b.telephone}")'
                style="display:flex;flex-direction:column;align-items:center;gap:6px;flex-shrink:0;background:none;">
                <div class="avatar avatar-md">${getInitials(b.nom, b.prenom)}</div>
                <span style="font-size:11px;color:var(--text-secondary);max-width:52px;
                             text-overflow:ellipsis;overflow:hidden;white-space:nowrap;">${b.prenom}</span>
              </button>`).join('');
      }
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* CORRECTION PRINCIPALE — Masquer le splash et démarrer l'app */
  ...
  /* Attendre la fin de l'animation de la barre (1.6s) + marge */
  setTimeout(startApp, 1800);
});
...
async function initRechargeView() {
  ...
  if (r.isFailure) {
    ...
    if (el) el.innerHTML = '<p style="color:var(--error);font-size:13px;">Impossible de charger les modes de paiement.</p>';
    return;
  }
...
function selectMode(modeId) {
  ...
  document.querySelectorAll('.mode-card').forEach(card => {
    ...
    if (radio) { radio.style.borderColor = ''; radio.innerHTML = ''; }
  });

  const sel = document.querySelector(`[data-mode-id="${modeId}"]`);
  if (sel) {
    sel.style.borderColor = 'var(--gold)';
    const radio = sel.querySelector(`#mode-radio-${modeId}`);
    if (radio) {
...
async function soumettreDemande() {
  hideError('rch-error');
  if (!_rchModeSelect) { showError('rch-error', 'Sélectionnez un mode de paiement.'); return; }
  ...
  if (!montant || montant < 10) { showError('rch-error', `Montant minimum 10 ${_rchDevise}.`); return; }
  if (!numTx || numTx.length < 4) { showError('rch-error', 'Entrez un numéro de transaction valide.'); return; }
    ...
    Modal.show(`
      ...
      <div style="text-align:center;padding:8px 0 20px;">
        ...
        <p style="font-size:14px;color:var(--text-secondary);line-height:1.6;">
          ...
          via <strong style="color:var(--text-primary);">${_rchModeSelect.label}</strong> a bien été reçue.<br><br>
          Délai : <strong style="color:var(--gold);">24 heures</strong> —
          ticket par <strong style="color:var(--text-primary);">email</strong>.
        </p>
      </div>
      <button class="btn btn-primary" onclick="Modal.hide();Router.go('home');">Retour à l'accueil</button>
      <button class="btn btn-ghost" onclick="Modal.hide();switchRechargeTabById('suivi');"
        style="margin-top:8px;font-size:13px;">Suivre ma demande</button>
    `);
  } else {
...
async function loadSuivi() {
  ...
  const demandes = r.data?.data || r.data?.results || [];
  if (!demandes.length) {
    el.innerHTML = emptyState('receipt', 'Aucune demande', "Vous n'avez pas encore soumis de demande.");
    return;
  }
    ...
    <div style="background:var(--surface);border-radius:var(--radius);padding:16px;margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
        ...
        <div style="background:${s.bg};color:${s.color};font-size:11px;font-weight:700;
                    padding:4px 10px;border-radius:20px;white-space:nowrap;">${s.icon} ${s.label}</div>
      </div>
      <div style="display:flex;justify-content:space-between;padding-top:10px;
                  border-top:1px solid rgba(88,94,101,0.3);">
        <div>
      ...
      ${d.motif_rejet ? `<div style="margin-top:8px;font-size:12px;color:var(--error);">Motif : ${d.motif_rejet}</div>` : ''}
    </div>`;
  }).join('');
}

function switchRechargeTabById(tabId) {
  const order   = ['demande','ticket','suivi'];
  const buttons = document.querySelectorAll('.tab-item');
  const idx     = order.indexOf(tabId);
  if (idx >= 0 && buttons[idx]) switchRechargeTab(buttons[idx]);
}